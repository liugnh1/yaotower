...
R.registerAll('curses', [
  ...
  { id: "forgetful",name:"健忘", desc: "技能消耗+5灵力，普攻伤害+40%",
    apply: p => { p._forgetfulOrigCost = p.mpCost; p.mpCost += 5; p._forgetfulAtkGain = Math.floor(p.atk * 0.4); p.atk += p._forgetfulAtkGain; },
    remove: p => { if (p._forgetfulOrigCost !== undefined) { p.mpCost = p._forgetfulOrigCost; delete p._forgetfulOrigCost; } if (p._forgetfulAtkGain !== undefined) { p.atk = Math.max(1, p.atk - p._forgetfulAtkGain); delete p._forgetfulAtkGain; } } },
