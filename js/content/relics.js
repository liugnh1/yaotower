...
R.registerAll('relics', [
  ...
  { id: "glass_cannon", name: "玻璃大炮", rarity: "legendary", desc: "伤害翻倍，但受伤也翻倍", icon: "💔",
    passive: p => { if (!p._glassCannon) { p.skillMul += 1.0; p._glassCannon = true; } }, onRemove: p => { if (p._glassCannon) { p.skillMul -= 1.0; p._glassCannon = false; } } },
  ...
  { id: "ninja_tabi",   name: "忍者足袋", rarity: "rare", desc: "闪避成功后下回合必定暴击", icon: "👣",
    passive: p => { if (!p._ninjaTabi) { p.dodge = (p.dodge||0)+0.05; p._ninjaTabi = true; } }, onRemove: p => { if (p._ninjaTabi) { p.dodge = Math.max(0,(p.dodge||0)-0.05); p._ninjaTabi = false; } } },
  ...
  { id: "core_shadow", name: "暗影之魂", rarity: "legendary", icon: "🌑",
    ...
    passive: p => { if (!p._coreShadow) { p.dodge = (p.dodge || 0) * 2; p._coreShadow = true; } },
