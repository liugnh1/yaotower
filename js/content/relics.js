...
R.registerAll('relics', [
  ...
  { category: "normal", id: "healing_tears",name: "治愈之泪", rarity: "common", desc: "击杀敌人恢复12%最大生命", icon: "💧",
    onKill: p => { p.hp = Math.min(p.maxHp, p.hp + Math.floor(p.maxHp * 0.12)); } },
  { category: "normal", id: "spike_shell", name: "尖刺外壳", rarity: "common", desc: "受击反弹40%伤害给敌人", icon: "🐚",
    onHit: (p, e, dmg) => { e.hp -= Math.floor(dmg * 0.40); } },
  ...
  { category: "normal", id: "thunder_clap", name: "雷霆掌", rarity: "rare", desc: "普攻15%概率眩晕敌人1回合", icon: "🤚",
    onAttack: (p, dmg, s, target) => {
      var chance = p._synThunderGod ? 0.50 : 0.25; // v0.81: 15%→25%, 雷神30%→50%
  ...
  { category: "normal", id: "fire_aura",   name: "烈焰光环", rarity: "rare", desc: "每回合对所有敌人造成ATK×15%+层数/2的燃烧伤害", icon: "🔥",
    onTurn: (p, e, s, allEnemies) => {
      var targets = allEnemies || (e ? [e] : []);
      targets.forEach(function(en) { if (en && en.hp > 0) {
        var d = Math.max(3, Math.floor((p.atk || 10) * 0.15) + Math.floor((s ? s.totalFloor : 1) / 2)); // v0.81: 10%→15%, /3→/2
        if (p._fireChain >= 1) d = Math.floor(d * 1.5);
        if (p._brandBurnDmg) d = Math.floor(d * (1 + p._brandBurnDmg));
        en.hp -= d;
      }});
    } },
  ...
  { category: "normal", id: "greed_bag",   name: "贪婪之袋", rarity: "rare", desc: "每持有30金币，攻击+3（上限+24）", icon: "💰",
  ...
  { category: "normal", id: "echo_stone",  name: "回音石",   rarity: "rare", desc: "每使用4次技能，第5次不耗CD不耗能量", icon: "🪨",
    passive: p => { p._echoStone = true; p._echoCount = 0; }, onRemove: p => { p._echoStone = false; } },

  // ===== 史诗 =====
  { category: "normal", id: "phoenix_feather",name:"凤凰羽", rarity: "epic", desc: "死亡时复活并恢复70%生命+全CD清零（每局1次）", icon: "🪶",
    passive: p => { p.rebirth = true; p._phoenixFeather = true; }, onRemove: p => { p.rebirth = false; p._phoenixFeather = false; } },
  ...
  { category: "normal", id: "soul_link",    name: "灵魂链接", rarity: "epic", desc: "你对敌人造成伤害时回复22%生命", icon: "🔗",
    onAttack: (p, dmg) => { p.hp = Math.min(p.maxHp, p.hp + Math.floor(dmg * 0.22)); } },
  ...
  { category: "normal", id: "doom_clock",   name: "末日时钟", rarity: "legendary", desc: "第5回合起对所有敌人造成%HP伤害(对Boss减半)", icon: "🕐",
    onTurn: (p, e, s, allEnemies) => {
      if (!s) return; var t = s.turnInFloor || 0; if (t < 5) return;
      var pct = t >= 10 ? 0.10 : 0.05; // v0.81: 20%→10%, 8%→5%
      var targets = allEnemies || (e ? [e] : []);
      targets.forEach(function(en) { if (en && en.hp > 0) { var dmg = Math.floor(en.maxHp * pct); if (en._isBoss) dmg = Math.floor(dmg / 2); en.hp -= dmg; } });
    } },
  ...
  // ===== v0.42 新增 =====
  { category: "normal", id: "chain_lightning",name:"连锁闪电", rarity: "rare", desc: "击杀敌人时对随机另一个敌人造成80%伤害", icon: "🔗",
    onKill: (p, s) => { if (s && s.enemies) { var others = s.enemies.filter(function(e){return e.hp>0;}); if (others.length>0) { var t = others[Math.floor((s.rng?s.rng.next():Math.random())*others.length)]; var dmg = Math.floor((p.atk||10)*0.8); t.hp -= dmg; } } } },
  { category: "normal", id: "golden_apple",  name: "金苹果",   rarity: "common", desc: "每拥有30金币，每回合回复2%生命", icon: "🍎",
    passive: p => { p._goldenApple = true; }, onRemove: p => { p._goldenApple = false; },
    onTurn: (p, e, s) => { if (s && s.gold > 0) { var appHeal = Math.max(1, Math.floor(p.maxHp * 0.02 * Math.floor(s.gold / 30))); p.hp = Math.min(p.maxHp, p.hp + appHeal); } } },
  { category: "normal", id: "mirror_shield", name: "镜盾",     rarity: "rare", desc: "受击时25%概率完全格挡该次伤害", icon: "🪞",
    passive: p => { p._mirrorShield = true; }, onRemove: p => { p._mirrorShield = false; },
    onHit: (p, e, dmg, s) => { return true; } },
  { category: "normal", id: "blood_ruby",   name: "血晶石",   rarity: "epic", desc: "每损失10%生命，攻击+3", icon: "💎",
    passive: p => { p._bloodRuby = true; }, onRemove: p => { p._bloodRuby = false; } },
  { category: "normal", id: "toxic_cloud",  name: "毒雾",     rarity: "rare", desc: "战斗开始时对所有敌人施加ATK×10%中毒3回合", icon: "☠️",
    passive: p => { p._toxicCloud = true; },
    onRemove: p => { p._toxicCloud = false; },
    onAcquire: (p, s) => { if (s && s.enemies && s.enemies.some(function(e){return e.hp>0;})) { s.enemies.forEach(function(e){ if(e.hp>0) e._buffs.push({id:'poison',name:'中毒',turns:3,data:{dmg:Math.floor((p.atk||10)*0.10)},onTick:function(em,b){em.hp-=b.data.dmg;if(em.hp<=0)return'dead';}}); }); } } },
  ...
  { category: "normal", id: "war_drum",     name: "战鼓",     rarity: "epic", desc: "每经过3回合，本场战斗攻击+6（可叠加）", icon: "🥁",
    passive: p => { p._warDrum = true; }, onRemove: p => { p._warDrum = false; } },
  { category: "normal", id: "cursed_doll",  name: "咒怨人偶", rarity: "epic", desc: "每持有一个诅咒，暴击伤害+20%", icon: "🪆",
    passive: p => { p._cursedDoll = true; }, onRemove: p => { p._cursedDoll = false; } },
  { category: "normal", id: "philosopher_stone",name:"贤者之石",rarity:"legendary",desc:"+33攻击 +33防御 +33生命", icon: "🪨",
    passive: p => { if (!p._philoStone) { p.atk += 33; p.def += 33; p.maxHp += 33; p.hp += 33; p._philoStone = true; } },
    onRemove: p => { if (p._philoStone) { p.atk = Math.max(1, p.atk - 33); p.def = Math.max(0, p.def - 33); p.maxHp = Math.max(1, p.maxHp - 33); p.hp = Math.min(p.hp, p.maxHp); p._philoStone = false; } } },
  { category: "normal", id: "angel_wings",  name: "天使之翼", rarity: "legendary", desc: "每场战斗首次死亡时复活并恢复20%生命", icon: "👼",
    passive: p => { p._angelWings = true; }, onRemove: p => { p._angelWings = false; } },
  { category: "normal", id: "medusa_head",  name: "美杜莎之首",rarity:"legendary",desc:"普攻5%概率直接石化敌人（即死）", icon: "🐍",
    passive: p => { p._medusaHead = true; }, onRemove: p => { p._medusaHead = false; } },
  { category: "normal", id: "gamblers_dice",name: "赌徒骰子", rarity: "rare", desc: "暴击率提升20%，但未暴击时伤害减30%", icon: "🎲",
